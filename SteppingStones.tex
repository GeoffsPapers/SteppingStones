...
from early on researchers collected problems in order to obtain
a basis for experimentation.
The first major publication\footnote{To our knowledge, the first circulation 
of problems for testing ATP systems was due to Larry Wos in the late sixties.}
in this regard was \cite{MOW76}, which provides 
an explicit listing of clauses for 63 problems, many of which are still
relevant today.
In the same year \cite{WM76} documented 86 problems which have since commonly 
been used for ATP testing. 
The problem clauses are not supplied in \cite{WM76}, however.
A second major thrust was provided by \cite{Pel86}, which lists 75 problems.
Other more recent papers are \cite{BLM+86}, \cite{Qua92}, 
\cite{MW92}, and \cite{McC93}, to name a few. 
...
This is a cumbersome, error-prone process, and is feasible for only very
small numbers of small problems. 
A problem library in electronic form was made publicly available by Argonne 
National Laboratories (in Otter format \cite{McC94}) in 1988 \cite{ANL}.
This library has been a major source of problems for ATP researchers.
Other electronic collections of problems are available, but have not been 
announced officially (e.g., that distributed with the SPRFN ATP system 
\cite{SPRFN}).

Prior to the development of the TPTP a large number of interesting problems had
accumulated in the ATP community. 
From early on researchers published particularly interesting individual 
problems, e.g., \cite{MOW76,Pel86-JAR,BL+86,Qua92-JAR,MW92-CADE-11,McC93}.
Problems published in hardcopy form are, however, often not suitable for 
testing ATP systems, because they have to be transcribed to electronic form.
This is a cumbersome, error-prone process, and is feasible for only small 
numbers of small problems. 
Researchers also collected problems in electronic form, e.g., in 1988 the 
Argonne National Laboratory produced a collection in Otter format 
\cite{McC03-Otter}, and the SPRFN ATP system \cite{Pla88} was distributed
with a collection of problems.
%To the author's knowledge, the first circulation of problems for testing ATP 
%systems was due to Larry Wos in the late sixties.
%The first major publication in this regard was \cite{MOW76}, which provides 
%an explicit listing of 63 CNF problems.
%In the same year \cite{WM76} documented 86 CNF problems that were commonly 
%used for ATP testing. 
%The problem clauses are not supplied in \cite{WM76}, however.
%A second major thrust was provided by \cite{Pel86-JAR}, which lists 75 FOF
%problems.
%Other early papers include \cite{BL+86}, \cite{Qua92-JAR}, \cite{MW92-CADE-11},
%and \cite{McC93}.
%The Journal of Automated Reasoning's Problem Corner also provided interesting 
%challenge problems.
%A problem library in electronic form was made publicly available by Argonne 
%National Laboratories in Otter format \cite{McC03-Otter}, in 1988.
% This library was a major source of problems for ATP researchers.
%Other electronic collections of problems were also available, but were not 
%announced officially, e.g., that distributed with the SPRFN ATP system 
%\cite{Pla88}.

Prior to the development of the TPTP a large number of interesting problems had
accumulated in the ATP community, in print form, e.g., 
\cite{MOW76,Pel86-JAR,Qua92-JAR}, and as electronic collections, e.g., the 
Argonne National Laboratory collection \cite{McC03-Otter} and the SPRFN 
collection \cite{Pla88}.
